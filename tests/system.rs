...
fn copy_to_tempdir(inputs: Vec<String>, tempdir: &Path)
{
    for f in inputs {
...
fn test_basic_compile(compiler: Compiler, tempdir: &Path) {
    ...
    copy_to_tempdir(vec![INPUT.to_string(), INPUT_ERR.to_string()], tempdir);
...
fn test_gcc_clang_no_warnings_from_macro_expansion(compiler: Compiler, tempdir: &Path) {
    ...
    // Compile a source file.
    copy_to_tempdir(vec![INPUT_MACRO_EXPANSION.to_string()], tempdir);

    let out_file = tempdir.join("test_macro_expansion.o");
    trace!("compile");
    sccache_command()
        .args([&compile_cmdline(name, &exe, INPUT, OUTPUT)[..], &vec_from!(OsString, "-Wunreachable-code")[..]].concat())
        .current_dir(tempdir)
        .envs(env_vars.clone())
        .assert()
        .success()
